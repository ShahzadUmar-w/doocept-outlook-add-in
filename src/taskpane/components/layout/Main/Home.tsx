...
const Home = () => {
  ...
  // Helper function to fetch children (shared between label click and arrow click)
  ...
  // ISSUE 4 FIX: Recursive search logic
  const getFilteredTree = (nodes: any[], query: string): any[] => {
    if (!query) return nodes;
    
    return nodes.reduce((acc: any[], node) => {
      const isMatch = node.name.toLowerCase().includes(query.toLowerCase());
      const filteredChildren = node.children ? getFilteredTree(node.children, query) : [];

      // If the node matches OR any of its children match, include it in results
      if (isMatch || filteredChildren.length > 0) {
        acc.push({ 
            ...node, 
            children: filteredChildren,
            // Automatically expand folders that contain search results
            isExpandedBySearch: true 
        });
      }
      return acc;
    }, []);
  ...
  const handleNext = async () => {
    if (!selectedFolderId) {
      setToast({ open: true, message: "Please select a folder first", severity: "error" });
      return;
    }
    setLoader(true);
    try {
  ...
  const renderTreeItems = (nodes: any[]) => {
    return nodes.map((node, index) => (
      ...
      >
        ...
        >
          {/* ISSUE 5 FIX: Render a placeholder if hasChildren is true so the arrow mark appears immediately */}
          {node.children && node.children.length > 0 
            ? renderTreeItems(node.children) 
            : (node.hasChildren ? <TreeItem itemId={`${node.uuid}-loading`} label="Loading..." sx={{ display: 'none' }} /> : null)
          }
        </TreeItem>
      </Grow>
    ));
  ...
  // Applying the recursive filter for Issue 4
  const filteredFolders = getFilteredTree(folders, searchQuery);
              ...
              <Slide direction="up" in={true} style={{ transitionDelay: "400ms" }}>
                <TreeView
                  ...
                  onItemExpansionToggle={(__event, itemId, isExpanded) => {
                    ...
                    // If expanding a folder via arrow that hasn't loaded data yet, load it
                    if (isExpanded) {
